...
产品以完全离线与内网部署为核心。当前文档先说明开发环境安装；完整离线安装材料与断网专项验收尚未完成，见 [离线部署要求](OFFLINE_DESIGN.md)。本机运行不等于已验证完全离线交付。

## 环境与安装

本次验证环境：Windows、Python 3.14.2、Node.js 24.12.0、npm 11.6.2。桌面模式使用 Windows WebView2。其他 Python 版本和操作系统尚未完成桌面验收。

以下命令默认从包源获取依赖，适用于可准备依赖的开发环境。在仓库根目录打开 PowerShell：
...
下述两个示例只使用本地数据和回环服务。安装与构建所需材料准备齐全后不需要业务公网账号；完全离线环境的专项验证仍以部署验收记录为准。
